...
**/
	...
	tBTNodePtr next_node = *RootPtr;
	tBTNodePtr parent_node = NULL;
	
	while(next_node != NULL)
	{
		parent_node = next_node; // always have to save parent node

		if(next_node->Cont > Content) // go left
		{
			next_node = next_node->LPtr;
		}
		else if(next_node->Cont < Content) // go right
		{
			next_node = next_node->RPtr;
		}
		else if(next_node->Cont == Content) // same content is already in tree
		{
			return;
		}
	}
	...
	if(parent_node == NULL) {(*RootPtr) = new_node;} // tree is empty, create root node

	else if(parent_node->Cont > Content) {parent_node->LPtr = new_node;}  // create left child node  
	
	else parent_node->RPtr = new_node; // create right child node
}
