...
void htInsert ( tHTable* ptrht, tKey key, tData data ) {
	...
	if(item != NULL) // key exists, renew data
	{
		item->data = data;
	}
	else // insert new key
...
void htDelete ( tHTable* ptrht, tKey key ) {
	...
	if(item == NULL){return;} //nothing to remove
	else if(item->ptrnext == NULL) //only one item is there
	{
		free(item);
		(*ptrht)[index] = NULL;
	}
	else if(item->ptrnext != NULL) //it's first but more items are there
	{
		tmp = item->ptrnext;
		free(item);
		(*ptrht)[index] = tmp;
