...
**/
	if(RootPtr != NULL)
	{
		...
		{
			if(K < RootPtr->Key) return BSTSearch(RootPtr->LPtr, K, Content);
...
**/
	...
	else if((*RootPtr)->Key == K) // rewrite
	{
		(*RootPtr)->BSTNodeCont = Content;
	}
	else if((*RootPtr)->Key > K) // go left
	{
		BSTInsert(&((*RootPtr)->LPtr), K, Content);
	}
	else if((*RootPtr)->Key < K) // go right
	{
...
**/
	...
	if((*RootPtr)->RPtr != NULL) {ReplaceByRightmost(PtrReplaced, (&(*RootPtr)->RPtr));} // recursively go to the most right leaf node
	else
	{
		PtrReplaced->Key = (*RootPtr)->Key;
		PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
		tBSTNodePtr tmp = (*RootPtr)->LPtr; // save left branch
		free(*RootPtr); // free node
		(*RootPtr) = tmp; // reconnect pointers on deleted place with left branch
	}
}
